...
const Toast = ({ message, type = "error", onClose, duration = 3000 }) => {
  ...
  return createPortal(
    <div className="fixed bottom-4 right-4 z-[300]">
      <div
        className={`rounded-lg shadow-xl p-4 flex items-center space-x-3 
        backdrop-blur-sm
        ${
          type === "error"
      ...
      >
        <AlertCircle
          className={`w-5 h-5 ${type === "error" ? "text-red-400" : "text-blue-400"}`}
        />
        <p className="text-sm font-medium">{message}</p>
        <button
          onClick={onClose}
          className={`p-1 rounded-full transition-colors ${
            type === "error" ? "hover:bg-red-100/80" : "hover:bg-blue-100/80"
          }`}
        >
          <X className="w-4 h-4" />
        </button>
