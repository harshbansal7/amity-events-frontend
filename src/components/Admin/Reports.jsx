...
const Reports = () => {
  ...
  const handleExport = async (format) => {
    try {
      ...
      const downloadFn =
        format === "pdf" ? downloadParticipantsPDF : downloadParticipantsExcel;
      const fileExt = format === "pdf" ? "pdf" : "xlsx";

      const blob = await downloadFn(selectedEvent._id, {
        fields_printed: selectedFieldsList,
      });
      ...
      <div className="bg-white p-6 rounded-xl shadow-sm">
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-500">
            Choose which information to include in the report
          </p>
        </div>
